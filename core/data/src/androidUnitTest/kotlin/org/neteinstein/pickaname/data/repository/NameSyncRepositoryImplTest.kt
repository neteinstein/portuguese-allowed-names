...
class NameSyncRepositoryImplTest {
    ...
    private val nameLocalDataSource: NameLocalDataSource = mockk(relaxed = true)

    private val repository = NameSyncRepositoryImpl(
        remoteDataSource = remoteDataSource,
        pdfTextExtractor = pdfTextExtractor,
        textParser = textParser,
        nameLocalDataSource = nameLocalDataSource
    )
    ...
    fun `successful sync purges and repopulates the database`() = runTest {
        ...
        coVerify(exactly = 1) { nameLocalDataSource.replaceAll(any()) }
    }
    ...
    fun `deduplicates parsed names before persisting`() = runTest {
        ...
        val slot = mutableListOf<List<NameRecord>>()
        coEvery { nameLocalDataSource.replaceAll(capture(slot)) } returns Unit
    ...
    fun `network failure while downloading maps to a NETWORK error`() = runTest {
        ...
        coVerify(exactly = 0) { nameLocalDataSource.replaceAll(any()) }
    }
    ...
    fun `a database failure while persisting is surfaced as INVALID_SOURCE, not a crash`() = runTest {
        ...
        coEvery { nameLocalDataSource.replaceAll(any()) } throws RuntimeException("disk full")
